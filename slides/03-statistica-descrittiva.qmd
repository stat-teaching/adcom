...
pdf: "[{{< fa file-pdf >}}](slides/03-statistica-descrittiva.pdf)"
filters:
  - bibentry
---
...
In caso di una distribuzione normale *perfetta* i punti dovrebbero stare sulla linea, come se avessere una correlazione perfetta. Deviazioni dalla linea indicano deviazioni da una distribuzione normale.

```{r}
par(mfrow = c(1, 2))

hist(dimarco2020$burnout, freq = FALSE, xlim = c(-1, 5))
curve(dnorm(x, mean(dimarco2020$burnout), sd(dimarco2020$burnout)), add = TRUE, col = "firebrick", lwd = 2)
qqnorm(dimarco2020$burnout)
qqline(dimarco2020$burnout, col = "firebrick", lwd = 2)
```
...
```{r}
ggplot() +
  stat_function(fun = pnorm, args = list(mean = 100, sd = 15)) +
  scale_x_continuous(name = 'x', 
                     limits = c(100 - 15 * 4, 100 + 15 * 4), 
                     n.breaks = 10,
                     sec.axis = sec_axis(~ (. - 100)/15, name = 'z')) +
  geom_vline(xintercept = 115, lty = "dotted") +
  geom_hline(yintercept = pnorm(1), lty = "dotted") +
  geom_point(aes(x = 115, y = pnorm(1)), color = "firebrick", size = 5) +
  ylab("Probabilità cumulata")
```

## QQ Plot

Vediamolo con la variabile `burnout` che ha media `r round(mean(dimarco2020$burnout), 2)` e deviazione standard `r round(sd(dimarco2020$burnout), 2)`. Con $z = 1$ ci aspettiamo (assumendo normalità) un valore di circa $x = 2.5$.
...
ggplot() +
  stat_function(fun = pnorm, args = list(mean = m, sd = s)) +
  scale_x_continuous(name = 'x', 
                     limits = c(m - s * 4, m + s * 4), 
                     n.breaks = 10,
                     sec.axis = sec_axis(~ (. - m)/s, name = 'z')) +
  geom_vline(xintercept = 2.5, lty = "dotted") +
  geom_hline(yintercept = pnorm(1), lty = "dotted") +
  geom_point(aes(x = 2.5, y = pnorm(1)), color = "firebrick", size = 5) +
  ylab("Probabilità cumulata")
```
...
```{r}
par(mfrow = c(1, 2))
hist(dimarco2020$burnout, breaks = 10, freq = FALSE, xlim = c(0, 5))
curve(dnorm(x, m, s), add = TRUE, lwd = 2, col = "firebrick")
qqnorm(dimarco2020$burnout)
qqline(dimarco2020$burnout, lwd = 2, col = "firebrick")
```

## Misure di forma

Le misure di forma ci forniscono informazioni riguardo la forma della distribuzione dei dati. Indici di tendenza centrale e dispersione possono essere influenzati dalla forma della distribuzione. Tendenzialmente la forma viene intesa come deviazione dalla simmetria di una distribuzione normale. Gli aspetti principali sono:
...
L'asimmetria indica una deviazione dalla simmetria della normale. Possiamo avere asimmetria negativa e positiva:
...
a <- 2
b <- 9
curve(dbeta(x, a, b), yaxt = "n", ylab = "", xaxt = "n", xlab = "Asimmetria Positiva")
abline(v = a / (b + a), col = "red")
abline(v = qbeta(0.5, a, b), col = "green")

a <- 5
b <- 5
curve(dbeta(x, a, b), yaxt = "n", ylab = "", xaxt = "n", xlab = "No Asimmetria")
abline(v = a / (b + a), col = "red")
abline(v = qbeta(0.505,a, b), col = "green")

a <- 9
b <- 2
curve(dbeta(x, a, b), yaxt = "n", ylab = "", xaxt = "n", xlab = "Asimmetria Negativa")
abline(v = a / (b + a), col = "red")
abline(v = qbeta(0.50,a, b), col = "green")

legend("topleft", legend = c("Media", "Mediana"), fill = c("red", "green"))
```

## Asimmetria

Il calcolo effettivo è più complesso e ci sono diversi modi di calcolarlo ([https://en.wikipedia.org/wiki/Skewness](https://en.wikipedia.org/wiki/Skewness)). In R invece è molto semplice usando il pacchetto `psych`:
...
x_lep <- rt(1e4, 6)
hist(x_lep, freq = FALSE, ylim = c(0, 0.4), breaks = 40, main = "Leptocurtica")
curve(dnorm(x), add = TRUE, lwd = 2, col = "firebrick")

qqnorm(x_lep)
qqline(x_lep, lwd = 2, col = "firebrick")

x_raw <- rbeta(1e4, 3, 3)
x_pla <- (x_raw - mean(x_raw)) / sd(x_raw)

hist(x_pla, freq = FALSE, breaks = 40, ylim = c(0, 0.5), 
     xlab = "", main = "Platicurtica")
curve(dnorm(x), add = TRUE, lwd = 2, col = "firebrick")

qqnorm(x_pla)
qqline(x_pla, lwd = 2, col = "firebrick")
```

## Curtosi

Il calcolo effettivo è più complesso e ci sono diversi modi di calcolarla ([https://en.wikipedia.org/wiki/Kurtosis](https://en.wikipedia.org/wiki/Kurtosis)). In R invece è molto semplice usando il pacchetto `psych`:
