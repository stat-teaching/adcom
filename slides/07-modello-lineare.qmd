...
## Predittore categoriale {.smaller}
...
- **Omoschedasticità.** La varianza dei residui deve essere costante. In pratica, la dispersione dei residui deve rimanere simile per tutti i valori predetti $\mu$ e, idealmente, per tutti i valori dei predittori $x$.
...
## Un caso ideale (positivo e negativo) {.smaller}
...
$$
y_i = \beta_0 + \beta_1 D_{i1} + \beta_2 D_{i2} + \dots + \beta_{k-1}D_{i,k-1} + \varepsilon_i
$$

$$
D_{ij} =
\begin{cases}
1 & \text{se l'osservazione } i \text{ appartiene al livello } j, \\
0 & \text{altrimenti.}
\end{cases}
$$
...
L'idea è la stessa dell'esempio con 2 gruppi. Sostanzialmente ogni parametro $\beta$ (a parte l'intercetta che vediamo dopo) rappresenta la differenza tra due livelli (proprio come nel caso dei tre gruppi).
...
Ogni parametro $\beta$ rappresenta quindi la differenza tra il gruppo di riferimento (intercetta) e il gruppo che quella specifica variabile dummy codifica. Sono sostanzialmente $k - 1$ t-test confrontati sempre con il gruppo di riferimento.
...
```{r}
ggplot(dat, aes(x = residence, y = attachment)) +
  geom_point(position = position_jitter(width = 0.1), size = 3) +
  geom_point(
    data = dat |> 
      group_by(residence) |> 
      mutate(attachment = mean(attachment)),
      size = 5,
      col = "firebrick"
  )
```

## Predittori categoriali con > 2 livelli {.smaller}
...
Stessa intepretazione del caso a due livelli, solo che abbiamo un contrasto in più.
...
ggplot(dat, aes(x = residence, y = attachment)) +
  ...
  geom_line(data = dat |> 
      group_by(residence) |> 
      mutate(attachment = mean(attachment)),
  aes(x = residence, y = attachment, group = 1)) +
  geom_point(
    data = dat |> 
      group_by(residence) |> 
      mutate(attachment = mean(attachment)),
...
#   annotate("label", x = 0.8, y = 3.9, label = latex2exp::TeX(r"($\beta_0$)", output = "character"), parse = TRUE,
# size = 5) +
#   annotate("label", x = 1.5, y = 3.9, label = latex2exp::TeX(r"($\beta_1$)", output = "character"), parse = TRUE,
# size = 5) +
#   annotate("label", x = 2.5, y = 3.9, label = latex2exp::TeX(r"($\beta_2$)", output = "character"), parse = TRUE,
# size = 5) +
  mtheme()
```
...
```{r}
dat_cs <- filter(dat, residence != "north")
dat_cs$residence <- factor(dat_cs$residence)
ggplot(dat, aes(x = period, y = attachment, color = residence)) +
  geom_point(size = 3)
```

## Predittori numerici e categoriali {.smaller}
...
Se intepretiamo i parametri come al solito, abbiamo:
...
Ma cosa significa *controllando per* con variabili categoriali e numeriche? Vediamolo graficamente concentrandosi (per semplicità) sul confronto sud vs centro.
...
dat_cs <- filter(dat, residence != "north")
dat_cs$residence <- factor(dat_cs$residence)
fit <- lm(attachment ~ period + residence, data = dat_cs)

b <- coef(fit)

line_data <- data.frame(
  residence = levels(dat_cs$residence),
  intercept = b["(Intercept)"] + c(0, b[paste0("residence", levels(dat_cs$residence)[2])]),
  slope = b["period"]
)
...
period_agg$attachment <- predict(fit, newdata = period_agg)

adj_mean <- data.frame(
  period = mean(dat_cs$period),
  residence = unique(dat_cs$residence)
)

adj_mean$attachment <- predict(fit, adj_mean)

ggplot(dat_cs, aes(x = period, y = attachment, color = residence)) +
  geom_point(size = 3) +
  geom_abline(
    data = line_data,
    aes(intercept = intercept, slope = slope, color = residence),
    linewidth = 1
  ) +
  geom_point(data = period_agg,
             aes(x = period, y = attachment),
             col = "black", size = 5, shape = 15) +
  geom_hline(yintercept = period_agg$attachment, lty = "dotted")
```

## Predittori numerici e categoriali

![](img/adj_mean1.svg){fig-align="center"}
...
ggplot(dat_cs, aes(x = period, y = attachment, color = residence)) +
  ...
  geom_point(data = period_agg,
             ...
             col = "black", size = 5, shape = 15) +
  geom_hline(yintercept = period_agg$attachment, lty = "dotted") +
  geom_vline(xintercept = mean(dat_cs$period)) +
  geom_hline(yintercept = adj_mean$attachment, lty = "dotted") +
  geom_point(data = adj_mean, aes(x = period, y = attachment, color = residence),
size = 5, shape = 15)
```

## Predittori numerici e categoriali

![](img/adj_mean2.svg){fig-align="center"}

## Medie aggiustate {.smaller}
...
![](img/adj_mean3.svg){fig-align="center"}

## Predittori categoriali, test omnibus {.smaller}
...
x <- runif(n, 0, 1)
g <- rep(0:1, each = n/2)
y_noint <- 0.5 + 1 * x + 1 * g + rnorm(n, 0, 0.3)
y_int <- 0.2 + 0 * x + 1 * g + 1 * x * g + rnorm(n, 0, 0.3)

data.frame(
    x, g, y_int, y_noint
) |> 
    pivot_longer(c(y_int, y_noint)) |> 
    mutate(name = ifelse(name == "y_int", "Interazione \n y ~ x * g", "No Interazione \n y ~ x + g"),
           g = ifelse(g == 0, "a", "b")) |>
    ggplot(aes(x = x, y = value, color = factor(g))) +
...
Nel modello additivo, non serve stimare la relazione `y ~ x` per ogni gruppo perchè assumiamo che sia la stessa. Infatti le rette sono parallele, basta stimarlo solo una volta.
...
g_{i} =
\begin{cases}
1 & \text{se l'osservazione } i \text{ appartiene al gruppo a }\\
0 & \text{altrimenti. (gruppo b)}
\end{cases}
$$

## Predittori categoriali e numerici

![](img/noint.svg){fig-align="center"}
...
Tecnicamente $\beta_3$ (il parametro di interazione) è **la differenza tra le pendenze dei due gruppi**.

Il parametro di interazione è $\beta_3$. l'interazione non è un modello adittivo perchè i due predittori si moltiplicano tra loro. Infatti si dice che è un modello **moltiplicativo** (o semplicemente con interazioni).

## Interazione in pratica {.smaller}
...
```{r}
dat |> 
  filter(residence != "north") |> 
  ggplot(aes(x = period, y = attachment, color = residence)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = FALSE)
```
...
Ora vediamo i parametri. C'è una differenzia sostanziale rispetto al modello additivo, l'effetto di un predittore viene condizionato all'altro. Nel modello additivo questo non è rilevante perchè si assume che non ci sia interazione. Metto tra [] l'aggiunta interpretativa.
...
- `period`: incremento di attachment per incremento unitario di period [quando residence è zero, quindi per le persone di centro]. In altri termini è la pendenza `attachment ~ period` per le persone di centro
...
#| fig-height: 10
library(ggeffects)
ggeffect(fit, terms = c("period", "residence")) |> 
  data.frame() |> 
  ggplot(aes(x = x, y = predicted)) +
  geom_line(aes(color = group)) +
  geom_point(data = filter(dat, residence != "north"),
aes(x = period, y = attachment, color = residence)) +
  xlim(c(0, max(dat$period))) +
  labs(
    y = "attachment",
    x = "period",
    color = "residence"
...
![](img/int.svg){fig-align="center"}

## Riassunto notazione modelli

Per riassumere la notazione dei modelli, sopratutto per distinguere modelli con e senza interazione:
...
## Test omnibus con interazione {.smaller}

Il test omnibus è particolarmente utile nel caso di predittori categoriali con più di due livelli ma anche in presenza di interazioni (sopratutto quando predittori categoriali a più livelli sono coinvolti).
...
In figura i parametri e la loro intepretazione dal grafico.

![](img/int3.svg){fig-align="center"}

## Test omnibus con interazione

In questo caso abbiamo due parameri di interazione, per averne uno unico possiamo usare `car::Anova()` (o test omnibus su Jamovi):
...
![](img/anovaint.svg){fig-align="center"}
