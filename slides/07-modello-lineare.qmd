...
Essendo l'argomento complesso e importante, solo in questo caso ci affidiamo anche a materiale più canonico. In particolare, le slide sono basate anche su questo capitolo di libro. Le cose da studiare sono quelle che facciamo a lezione MA nel libro le stesse sono spiegate in modo più discorsivo.

<center>

[LEARNING STATISTICS WITH JAMOVI - Capitolo 12](https://davidfoxcroft.github.io/lsj-book/lsj-free.pdf)
...
Quando abbiamo parlato di *correlazione*, abbiamo chiarito che due variabili sono correlate se la loro relazione è di tipo *lineare*. Lineare significa che la relazione può essere descritta da una retta. **Ma come disegnamo questa retta?**
...
for(i in 1:length(b1l)) {
    lp <- (b0l[i] + b1l[i]* dat$satisfaction)
    SS <- sum((dat$satisfaction - lp)^2)
    plot(dat$burnout, dat$satisfaction, type = "n", main = sprintf("SS = %.2f", SS))
...
text(x = 0, y = b0-0.2, label = TeX(r"($\beta_0$)"), cex = 1.5)
...
Vediamo dei casi dove la pendenza/slope della retta è la stessa ma la deviazione standard dei residui cambia.

```{r}
n <- 100
x <- runif(n, 0, 10)

y1 <- 2 + 0.5 * x + rnorm(n) * 0.2
y2 <- 2 + 0.5 * x + rnorm(n) * 1
y3 <- 2 + 0.5 * x + rnorm(n) * 2
...
Questa statistica test sotto $H_0$ si distribuisce come una $t$ di Student con $n - p$ dove $p$ è il numero totale di coefficienti stimati dal modello. Nel caso dell'altezza, $p = 2$ ($\beta_0$ e $\beta_1$) quindi $\nu = n - p = 396 - 2 = 394$.

# Regressione multipla

## Regressione multipla

Un modello di regressione passa da essere semplice a multiplo quando il numero di predittori è $> 1$. Da non confondere dalla regressione multivariata dove il numero di outcomes è $> 1$.

La logica (minimi quadrati, inferenza, $R^2$, residui, etc.) è esattamente la stessa a prescindere dal numero di predittori MA, ci sono dei punti importanti:

- la visualizzazione grafica diventa complessa se non impossibile
- i coefficienti associati allo stesso predittore possono cambiare nel momento in cui aggiungiamo un nuovo predittore
- l'intepretazione dei parametri diventa più complessa
- si aprono problemi di parsimonia del modello nei termini di un trade-off tra complessità (numero di parametri)
...
# 1. Modello lineare
model <- lm(math ~ peso + age, data = dat)

# 2. Griglia per il piano (X=peso, Z=age, Y=math)
axis_x <- seq(min(dat$peso, na.rm=T), max(dat$peso, na.rm=T), length.out = 30)
axis_z <- seq(min(dat$age, na.rm=T), max(dat$age, na.rm=T), length.out = 30)

surface_data <- expand.grid(peso = axis_x, age = axis_z)
surface_data$math <- predict(model, newdata = surface_data)

# Trasformiamo in matrice (Z per plotly è l'altezza, quindi qui mettiamo 'math')
math_matrix <- matrix(surface_data$math, nrow = 30, ncol = 30)

# 3. Grafico 3D ottimizzato
plot_ly(dat, x = ~peso, y = ~age, z = ~math) %>%
  add_markers(marker = list(size = 4, opacity = 0.6, 
                            color = ~age, colorscale = 'Viridis'), 
              name = "Studenti") %>%
  add_surface(x = ~axis_x, 
              ...
              showscale = FALSE, 
              opacity = 0.4, 
              name = "Effetto Combinato") %>%
  layout(scene = list(
    xaxis = list(title = 'Peso (Predittore)'),
    yaxis = list(title = 'Età (Confounder)'),
    zaxis = list(title = 'Math (Outcome)')
  ))
```
